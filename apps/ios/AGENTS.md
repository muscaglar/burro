...
- A source of a fact may bring a credit: the statement its publisher asks to see wherever a figure made from its data is shown (`FactSource.attribution`), and what its terms ask to be said with it (`saidWithAttribution`). Which source brings one is the API's to say. `SourceLines.credit` makes one line of both, as the website's `creditOf` does, and every line that ends a sentence or a figure says it after the name of its source and before the date: `SourceLine.words`, on the screen of an area and on the results. Under one figure a statement is said once, however many sources bring it. A source that brings none has none: the app writes no credit of its own. Draw no figure of a fact without its line.
...
- A test reads from the recording what the service made, and what says which build answered: the id of a share (`Answers.shareId`), the release and the engine (`Answers.meta`), the hash of a spec, and how many areas an answer holds. It never holds one as written. An id is made again each time the answers are recorded, so a test that held one failed, or looked for an id that nothing had made. `RecordedAnswersTests` fails on a file of tests that holds one. A test still names the area or the place it speaks of, as it names a recording, and still says in full a figure or a sentence that the app draws.
