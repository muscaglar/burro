...
| `src/lib/api/server.ts` | The API as a build reads it: routes 4, 5, 6 and 11 only. Never route 13 |
...
| `test/recorded/` | Answers captured from the real API. Written by `test/record.py`, never edited. `area/` holds every area's profile, `visit/` one person's whole visit, and `one-number/` a release whose prices have no range |
...
- Nothing is ranked from what the reader noticed until the person chooses. A choice sends the `operations` the API gave with it, unchanged. Every thing there is one way to want is in sight, so that one button adds them all: what waits behind "Show all" is only what is a question.
