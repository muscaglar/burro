 ...
 *
 * A path is made from a slug, a source id or nothing. None takes anything a
 * person typed, a place, a spec or a fact. A share's id goes in the fragment,
 * which a browser sends to no server.
 */
...
export type MethodsPart = "confidence" | "journeys" | "words";
...
export const paths = {
  ...
  /** One area's page. The city comes from the area's id. */
  area: (area: { readonly area_id: string; readonly slug: string }) => {
    const city: City | null = cityOf(area.area_id);
    if (city === null) throw new Error("Not an area id.");
    return `/${city}/${slugOr(area.slug, "a slug")}`;
  },
