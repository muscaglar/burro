...
| When | What | Note |
|---|---|---|
| The privacy notice, the ICO registration and the provider's agreement are in place, quotas exist, and the 150 golden queries pass | The model key | `fly secrets import --app APP`, then type `ANTHROPIC_API_KEY=` and the key, then end the input. It reads the key from the keyboard, so the key is not kept in the shell's history. Set a spend limit on the provider's workspace first |
...
| Model provider, later | The prompt, when a key is set | Inputs, for up to 30 days, longer if flagged: `docs/research/models/claude.md`, section 7, gives the pages | No UK or EU region: the same | 30 days | Nothing until the conditions in step 5 are met. The privacy notice must say this |
