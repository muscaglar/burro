...
| Part | State |
|---|---|
| Licence registry and ingest gate | Built. 126 sources are registered |
| Ranking engine, release format, API | Built. The catalogue holds 47 features and eleven vibes. An area with no figure for what was asked for stands below every area that has one, and says which figure it lacks. No database, no sign-in, no quotas |
...
| A preview of London | Built, and not served. The statistics office's 1,002 areas, with 28 measures and a band on eight of the eleven vibes. It holds no journey, no cost, no station and no name of a neighbourhood. It was made outside a hosted workflow, from a store that is a folder |
...
|---|---|---|---|
| 1 | Journey times | Most people begin with where they must get to. Until Burro holds journey times it can answer that part of no search. It says that journeys are not in its data, and ranks on the rest | An account with the transport authority for its timetables, the rail timetable, and a routing engine where builds run. The step that works journeys out is built, and has routed the made-up town alone |
...
| 3 | Prices, fetched for display | A budget is the second thing most people say. With no price Burro can test no budget and can show nothing of what homes sell for | The workbook of median prices has a receipt for checking alone. It is fetched again for display (ADR 0021) |
| 4 | A second try at Village feel | "A village feel" is among the first things people ask of a place. As built, the vibe does not find villages, so it is not served | The two measures of a town centre, its size and its shape, are built again once the founder has looked at the maps |
| 5 | Vibes from the age of residents and the make-up of households | A family and a student want different streets. These are the only figures of residents that may be ranked on | The measures are built and are in no release. Core, the reader, the contract and the website change together, and the row of the proxy audit is written first: [the design](design/residents-age-and-households.md) |
...
| 7 | A second source for pubs | "Pubs" is asked for by name, and Going out counts none today | The food register follows how a council fills it in as much as it follows pubs. A second source confirms them before they count |
| 8 | Deployment | Nobody can use what is not hosted | No object store exists, no hosted workflow has run, and no address serves the API or the website: [the account of data builds](data-builds.md) |
...
| Begun | The neighbourhood map: a draft, which no person has checked. The proxy audit: its first row, for recorded incidents |
...
| A steering or discrimination complaint | Legal and reputational | Rank places, and of residents only age and household make-up. Show ethnic group and religion and never rank on them. Read no wish for fewer of any group. A term that forbids steering. Code allowlist, crime off by default, audit with a written rule |
