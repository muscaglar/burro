...
| 3 | Rents, and the founder's word on what was built of prices | A budget is the second thing most people say. A build holds what each kind of home sold for, worked out from the sales of three years, and tests a budget to buy against it. It holds no rent, so it tests no budget to rent | No rent is published for an area as small as Burro's. The share of homes in the higher council tax bands and what homes sell for each follow household income, and whether they stay is the founder's to decide (ADR 0021 and 0028) |
| 4 | A second try at Village feel | "A village feel" is among the first things people ask of a place. As built, the vibe does not find villages. Since independent places were measured it holds 60 in 100 of its recipe, which would be enough for a band, and on that it finds inner London's old streets and not villages. So core places it only where the size or the shape of a town centre has a figure, and no build places an area on it | The two measures of a town centre, its size and its shape, are built again once the founder has looked at the maps. It is served once that second try reads as villages |
| 5 | Vibes from the age of residents and the make-up of households | A family and a student want different streets. These are the only figures of residents that may be ranked on | They are built: Family area and Young professionals, each offered and never applied. The row of the proxy audit is written, and no audit has run. What was chosen where the design left a choice is the founder's to overturn: [the design](design/residents-age-and-households.md), sections 11 and 13 |
...
| Begun | The neighbourhood map: a draft, which no person has checked. The proxy audit: its first row, for recorded incidents, and its second, for the age of residents and their households. No audit has run |
