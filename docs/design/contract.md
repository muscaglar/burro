...
`population` gives the figures of a measure among the rankable areas that have one, from the least, and `band` the band of one area's figure among them. A release works both out once, as it is made, for every measure it carries: the facts of an area ask for them of every figure, and working each out over every area again made a page of reasons take seconds on a city of a thousand areas.
