...
| `release/cli.py` | `burro-release build-synthetic --out FOLDER [--gritty a\|b] [--census-out FOLDER]` and `burro-release check FOLDER [--census FOLDER]` |
...
| `catalogue.json` | `{"catalogue_version": 11, "metrics": [...], "vibes": [...]}` | a feature this release carries, and a vibe it carries | 2.3 |
...
| `label`, `short_label`, `dimension`, `unit`, `polarity`, `kind`, `describes`, `family`, `method`, `in_likeness` | | Must equal `FEATURES` in core. Core decides these; the release repeats them so it can be read alone |
...
| `confidence` | `high`, `medium`, `low`, `unstated` | Of a range. High: at least 50 observations. Medium: 10 to 49, blended. Low: modelled. Of one number: `unstated` |
| `as_of`, `source_ids` | month, list of str | As in 2.3. Of one number, `as_of` is the last month of the twelve the sales were made in |

**A cost is a range, or one number where no range is known.** A row is one of two things, and says which by what it holds.
...
| One number | The median, and `null` for both quartiles | `unstated` | A publisher's own median of what was paid for the homes of one kind that were sold in the twelve months that end with `as_of`. It is carried as the publisher wrote it, to the pound, and is not rounded |

One number is never made into a range, and nothing is put in the place of a quartile that no source gives. It is `unstated` because the publisher gives a figure with no count of the sales behind it, so nothing says how steady it is. It is a price of every size of home of its kind: no source gives a price by bedrooms, and none is worked out. A rent is never one number: no source gives one for an area, and core refuses a rent with no range (`values_are_in_range`).

A real release holds no rent. Rents are published for boroughs and for postcode districts and not for areas, and no design holds the model that would bring one to an area. A renter's budget is then turned away as `not_in_release` (section 5.3, rule 15), and no area is given a rent that was not published for it. [ADR 0021](../adr/0021-a-price-is-shown-as-the-publisher-gives-it.md) says why a price is shown, and why as one number.
...
A preview that has routed no journey holds no destination: `destination_ids` is empty, every row of every matrix is empty, and the file states no source (section 2). `Release.travel()` then answers `missing` for any journey asked of it. `cutoff_minutes` must still be given, because the limits of a form are read from it (section 4). It then limits nothing, and what it holds is the builder's to say: it is not yet a thing a release can leave out (section 13).
...
| `catalogue_matches_core` | A feature is outside the allowlist, or disagrees with `FEATURES`, or is said to be ranked on where core says it is shown and never ranked on |
| `vibes_match_core` | `vibes` is not `tags_of(gritty_variant)`: a vibe is missing, extra, or disagrees with `TAGS` |
| `rows_are_complete` | `features.json` lacks a row for an area and a carried feature, `tags.json` for an area and a carried vibe, `geometry.json` lacks an area, or `travel.json` lacks an area or a destination |
| `values_are_in_range` | A percentile, coverage, quartile order, cutoff or minute value breaks its range. A cost holds one quartile and not the other, or holds none and is a rent or says any confidence but `unstated`, or holds both and says `unstated`. A figure of a measure whose unit is `%` is below 0 or above 100, or a figure of any other unit is below 0. A point of an area's outline, or the point inside a neighbour, lies more than half a degree from the point inside the area |
...
| `raw_matches_recipe` | A vibe's `raw` or `coverage` is not what `tag_raw` gives for the area's own percentiles in `features.json`, by the recipe of core |
...
| Shortest journey | No time in the release is under 2 minutes: not a cell of `travel.json`, and not a `walk_minutes` in `stations.json`. The `station_walk` feature is a distance in metres: the made-up town has no streets, so it is the walk to the nearest station at 80 m a minute, and none is under 160 m. A time is rounded to whole minutes and then held at 2, by `whole_minutes()` in `journeys.py` and nowhere else. The map can put a place at the middle of its area, and no distance at all is still down the stairs and across the road. Every sentence says "minutes", which 0 and 1 do not read well in |
| Gaps on purpose | About 5% of feature values are null. One rankable area has no cost rows. One has a `null` travel cell and one a `-1`. Otterby Fields, the new town, has none of the 17 features that version 2 added, so it can be placed on Houses or flats and on no other vibe. Grapnel Dock and Sedgewater Marsh, which have too few homes, have no share of homes and no rate, so neither can be placed on Quiet streets, Age of buildings, Everyday on foot, Houses or flats or Street character. These keep the missing-data paths tested |
...
47 features. Each describes a place, its buildings, or what was recorded there. None describes who lives there. An id names the idea, not the method: distances and thresholds live in `definition` and may change with `CATALOGUE_VERSION`. An id is never renamed, reused or given a new meaning.
...
| `kind` | `taste`: wanting less of it is a taste in places. `amenity`: a thing to be near. `nuisance`: a thing it is a nuisance to have, so its one direction is less. `on_request`: weighed only when a person asks in so many words. `NUISANCES` is read from it |
| `describes` | `place`, `buildings`, or `events`, which is what was recorded there. Nothing describes residents, and the enum has no member for them |
| `family` | One of the four families a setting is grouped by, or none: `streets_homes`, `pace_food`, `green`, `daily_life` |
...
| `venue_evening` | venues_culture | Pubs, bars and evening venues | Pubs and bars | per km² | either | taste | place | pace_food | yes | more / fewer | `fsa-food-hygiene-ratings` |
...
| `independents_nearby` | venues_culture | Independent places to eat and drink within a 10-minute walk | More independent places nearby | count | more | taste | place | pace_food | yes | more / fewer | none named yet |
...
| `evening_cluster_exposure` | air_noise | Share of homes within 150 m of a cluster of evening venues | Away from late venues | % | less | nuisance | place | pace_food | no | more / less | none named yet |
...
| `grocery_walk` | services | Walk to the nearest food shop | Nearer a food shop | min | less | amenity | place | daily_life | yes | further / closer | none named yet |
...
| `private_outdoor_space` | homes | Homes with private outdoor space | More homes with outdoor space | % | more | amenity | buildings | streets_homes | no | more / fewer | none named yet |
...
| `gp_walk` | services | Straight-line distance to the nearest GP practice, placed by its postcode | Nearer a GP surgery | m | less | amenity | place | daily_life | yes | further / closer | `nhs-ods-gp-practices`. No build carries it yet |
| `pharmacy_walk` | services | Straight-line distance to the nearest pharmacy, placed by its postcode | Nearer a pharmacy | m | less | amenity | place | daily_life | yes | further / closer | `nhsbsa-consolidated-pharmaceutical-list`. No build carries it yet |
...
- Walk distances are computed on a network that is not OpenStreetMap (ADR 0004). No such network is built yet. So `park_proximity` and `park_large_proximity` are straight lines, from the point where the homes of an output area are taken to stand to the nearest way in that the publisher marks, and their names say so. Since catalogue version 12 the same is so of `play_space_proximity`, `station_walk`, `highstreet_access`, `gp_walk` and `pharmacy_walk`, each a distance in metres, and of `school_primary_nearby`, which counts the schools within 800 m in a straight line. Each becomes a walk, under a name that says a walk, on the day a walk is worked out. Three measures keep a name that their figure does not yet bear out, so no build carries them: `park_facilities`, until it is decided whether a playing field is a park, and `centre_small` and `centre_compact`, so that Village feel stays off the map.
...
`raw = sum(w_i * u_i) / sum(w_i)` over the parts whose feature has a value, and `coverage = sum(w_i present)`. If `coverage < 0.6` then `raw`, `score` and `band` are null. `score` is the percentile of `raw` and `band` its band, by section 2.4. `tag_raw()` in core is the only implementation; the pipeline calls it. It rounds `raw` to six decimals, the precision a release is written with, so the number is the same before writing and after reading.
...
| `village_feel` | Village feel | streets_homes | one way | 25 `independents_nearby` high + 20 `centre_small` high + 20 `centre_compact` high + 20 `homes_pre1919` high + 15 `conservation_cover` high | 2: "villagey", high |
| `pace` | Going out | pace_food | scale, Calm to Buzzy | 45 `venue_food_drink_per_homes` high + 30 `highstreet_access` low + 25 `culture_venues_per_homes` high. The town centre is a distance, read from its near end. Pubs and bars, `venue_evening`, were 35 of it and are held back: they join when a second source confirms them | 3: "lively", high |
| `quiet_residential` | Quiet streets | streets_homes | one way | 40 `road_major_exposure` low + 30 `evening_cluster_exposure` low + 30 `noise_exposure` low | 4: "quiet street", high |
...
| `everyday_on_foot` | Everyday on foot | daily_life | one way | 25 `grocery_walk` low + 25 `highstreet_access` low + 20 `station_walk` low + 15 `gp_walk` low + 15 `pharmacy_walk` low | 6: "walkable", high |
...
|---|---|---|---|
| `b` | Eleven: the ten and Gritty. Works and warehouses is a part of Gritty, and is not carried | A wish for Gritty by name, and so for the recorded crime it counts. A word for works and warehouses, "industrial", is read into Gritty and offered | Any release. It is what the committed release and a release of London carry |
| `a` | Eleven: the ten and Works and warehouses. It holds no recorded crime, and no Gritty | Works and warehouses, as assumed, with `street_cleanliness` in `unmet` | Any release. It is built on demand, and no release that is served is one |
...
| `over_budget` | Budget is `hard`, an estimate exists, and what the budget is held against is over `amount`: the upper quartile of a range, and the median of a cost with no range (section 6.5) |
...
|---|---|---|---|---|
| `commute` | There is a commute and `commute_weight > 0` | `commute_weight` | 6.3 and 6.4 | No destination has a time |
...
What that means for a firm budget is for the founder to weigh (section 13). Half of the homes that were sold went for less than the median. So an area that a firm budget leaves out is one where the middle price was over the budget, and not one where no home was within it.
...
| `BudgetFit` | `upper_quartile` (`None` where the cost has no range), `margin` (to what the budget was held against, section 6.5), `utility`, `confidence`, `as_of` |
...
| What it may never be counted on | `may_be_compared()` refuses, whatever the flag says: a feature of `kind` `nuisance` or `on_request`, one that `describes` `events`, and one with no family. So recorded crime and recorded incidents are never in it. Nor is cost, a journey or a vibe's own score, which are no features. `homes_flats`, `homes_density`, `private_outdoor_space` and the school features are held out until an audit has passed them. Nothing about who lives somewhere can be in it: there is no such feature |
...
| `template` | enum | Which template of 7.3 states this fact. `budget_under` and `budget_over`, or the nearest station and another, cannot be told apart from the slots alone. A cost with no range has templates of its own, so that a client never draws a range from one number |
...
| `sources` | tuple of `FactSource` | `source_id`, `name` and `publisher`, from the manifest, in `source_id` order. Never empty |
...
| `cost` | tenure and segment with an estimate | `<tenure>.<segment>` | the three quartiles, or the median alone where the row has no range, and the year and month of `as_of` | none | the cost row |
| `budget_fit` | spec with a budget and an estimate | `<tenure>.<segment>` | amount, what the budget was held against (the upper quartile, or the median where the row has no range), `abs(margin)` | none | the cost row |
| `travel` | commute in the spec, unless the time that is scored is `missing` | `<place_id>.<mode>` | each of the two times that is `ok`, the cutoff when the scored time is beyond it, and the limit and the minutes over it when it is over | place name | `origin(travel)` |
| `station` | station row | the station id | walk minutes | station name, each line name | `origin(stations)` |
| `missing` | component the spec requests that is missing for the area, and each journey of the spec that has no time | the component name, or `commute.<place_id>.<mode>` | none | area name, and the place name for a journey | `origin(neighbourhoods)`, or `origin(travel)` for a journey |
...
Numbers are formatted once, here: money with thousands separators, metres to the nearest 10 and with thousands separators, "1,080 m", percentages to whole numbers, other values to one decimal with a trailing `.0` dropped. A share of areas is rounded down to a whole number and never to the nearest, so that a sentence never says more than is so. The mid-rank percentile an area is scored on is not among a fact's numbers and is never printed in a sentence: where areas tie it counts half of them as beaten, which is right for a score and untrue of the release.
...
| `budget_over_median` | The middle price of {homes} of all sizes is £{margin} over your budget of £{amount}. Used where the row has no range |
...
| `travel_pt_over`, `travel_other_over` | The same, ending: , {margin} {margin_unit} over the {limit} you set. Used when the time that is scored is over the journey's limit. `{margin_unit}` is "minute" or "minutes" |
...
|---|---|---|
| `grocery_walk` | 10 minutes | Ten minutes on foot is the catalogue's own measure of what is within a walk: `station_lines` and `independents_nearby` each count what is within a 10-minute walk, and a release lists a station as nearby within ten (section 2) |
| `park_proximity`, `play_space_proximity`, `station_walk`, `highstreet_access`, `gp_walk`, `pharmacy_walk` | 800 m | What a person walks in ten minutes, at 80 m a minute. Each is a straight line, so the walk is longer than the figure: the floor was chosen for a walk, and is to be looked at again |
...
- **Missing:** one sentence for each dropped component, and one for each journey that has no time, each citing its own `missing` fact.
...
| `others` | Placed, and in neither list: near the middle, beyond the three of a list, or a vibe the release does not let a result show |
...
| `InterpretResult` | `status`, `operations`, `assumptions` (each a `code`, `group`, `index`), `unmet` (tuple of `UnmetCategory`), `clarify` (each a `group`, `index` and up to five options of `id`, `name`, `kind`), `notice` (`none`, `neutral_places`, `off_topic`), `interpreter` (`rule`, `model`), `degraded`, `usage` (`input_tokens`, `output_tokens`, `cache_read_tokens`), `rests_on` (each a `group`, `index`, `start`, `end`), `suggestions` (tuple of `Suggestion`), `unread` (tuple of `Span`), `not_in_release` (tuple of `NotInRelease`, each a `target`, a `label` and `spans`) |
...
| A token | What stands between two spaces, less the marks around it. It is never split at a mark inside it: "don;t" is one token, and one the grammar does not place. A word typed with a hyphen is read whole or not at all |
...
- **What Burro has no measure of is offered what is nearest, and says what it lacks.** The first words a newcomer reaches for led nowhere: each was met with no edit, no suggestion and a line about settings. Each is now a phrase of the lexicon that is never applied. A prompt that holds one is not plain, what is nearest is offered as a choice, and the `note` of the suggestion says what Burro cannot do and why. `Target.note` in `lexicon.py` holds each.
  ...
  | "gym", "gyms", "leisure centre", "sports centre", "swimming pool" | `feature:park_facilities`: "More to do in parks" | "Burro has no data on gyms, pools or leisure centres yet. The nearest it can count is what there is to do in parks within a walk." |
  | "community", "sense of community", "community feel", "community spirit", "neighbourly" | `tag:village_feel`: "Add Village feel" | "Burro cannot measure whether neighbours know each other. The nearest it can count is a small centre of its own, with old streets and independent places." |
  ...
  | "affluent", "posh", "well heeled", "upmarket", "smart", where the release carries Gritty | Two, for the person to choose from: `tag:street_character`, with the one choice "Towards Polished, counting recorded crime", and `feature:price_median`, "What homes sell for", with the one choice "Dearer" | "Burro measures places, not the people in them." and, of the scale, what it counts |
  | "identity", "a real identity", "character", "characterful", "its own feel", "a proper neighbourhood", "soul", and "soulless" and "bland" | Three, for the person to choose from: `tag:village_feel` "Add Village feel", `tag:built_age` "Towards Historic" and `feature:highstreet_access` "Nearer a town centre" | "Burro cannot measure the character of a place. The nearest it can count are a village feel, the age of the buildings and a town centre nearby. Choose any that fit what you mean." |
  | "up and coming", where the release carries Gritty | `tag:street_character`, with both its ends. `change_over_time` is in `unmet` | "Burro cannot see how a place is changing. The nearest it can say is where a place stands on this scale today." and then what the scale counts |

  **A word for a smart area is read as of the place, and never of who lives there.** Decided on 2026-09-24: "affluent" and its kin have two readings, both of the place. One is the polished end of Gritty. The other is homes that sell for more than the middle of the city. Neither is what people earn or who they are, and no id exists that could express either of those (section 3.1). The first is offered, one way, where the grammar makes the sentence the word stands in: a person who types "slightly affluent" is not offered Gritty. In any other sentence nobody can say which way the word is meant, "not posh", "anything but posh", "posh is not for me", so both ends are offered, and the end that was meant is among them. **The second is offered beside the first.** `price_median` is the median price paid for a home of any kind, as its publisher gives it for the area. It is offered one way where the first is, "Dearer", and both ways where the first is, "Dearer" and "Cheaper". It is never applied from a word, it stands in no vibe, no likeness is counted on it, and nothing weighs it by default. Where a release carries no Gritty it is the one reading of the word, and "smart" is a word for upkeep there. Where a release holds no price, the offer is listed with what the release cannot answer. A word for people beside it is still a request about who lives somewhere, and is offered nothing: "posh people", "affluent families", "upmarket residents", "posh locals", "affluent households" (section 8.4).

  **A word for character is offered three ways, whatever is said of it.** "Soulless" and "bland" name a place without character, and "not bland" turns that round again. None is a wish for the opposite of a village feel, and nobody can say which of the three a person means by any of them. So each is offered the same three, one way, under a word that turns as without one, and "I don't care about character" takes nothing off. A word that is offered rests on all that was said of it, "slightly affluent", "a bit of character", where the grammar makes the sentence it stands in.
  ...
  **A suggestion that holds a note is chosen by its own label, and never with others at once.** The note is what a person should know before they choose, so a client that offers to add several things in one press leaves out every suggestion whose `note` is not empty. For recorded crime this is the rule of section 5.3 and not a courtesy: a button that adds "all 5" names no crime, so to press it is not to ask for crime by name. A client shows the `note` beside the choice, and a choice of recorded crime only under its own label.
...
1. **The rules read first.** A plain prompt is applied by the rules, and no call is made. Nor is one made where the rules made something of every word of a prompt that is not plain, as of the name of a scale alone: `asks_a_model` is whether `unread` holds anything.
...
| 5 | A scale with no end | The model named a scale and no end of it, or turned an end down. Or no phrase of core's names an end of the scale in the clause, whichever end the model chose: "the pace of the place", "the age of the buildings" | Both ends, and no guess |
...
| Anything about who lives somewhere, and any reading of a word about wealth | ADR 0006. The rules offer what is nearest for such a word, and what they offer stands. **Core lists the words about wealth and about identity.** It offers two readings of the one and three of the other, all of the place. A model adds no reading of "affluent" or of "a real identity", and a test holds each |
...
| What follows | `follows`, `said`, `note` | What happens to areas: which rank higher, which are left out. What a thing counts, and what a vibe cannot see. What nobody said, and what Burro took: "You named no way of travelling: Burro took public transport.", "You gave 35 to 40: Burro took 40." |
...
|---|---|
| A wish or a vibe, at a mention or a small step | Anything that leaves areas out: a firm limit, a rule for an area. Where the guess is a firm limit, it adds the guide |
| The tenure | An offer with two ways and no guess marked |
| A budget, as a guide | A journey to a place that is yet to be chosen |
| A journey, as a guide, to a place named in full | Recorded crime, in any form, and anything the rules offer with a note |
...
On the first look of the 112 sentences whose answers are kept, the guard offers 84 rightly, where the rules alone read 61. One backwards reading is marked as the guess. Nothing is applied, nothing is offered that is never to be offered, and no reading of the rules is lost. That is a fit and not a measurement: the checks were chosen after reading these answers.

**The floor does not hold against a model that means harm, or is careless.** A stand-in that raises whatever a sentence names is marked as the guess, backwards, on 88 of the 781 cases of the evaluation set. So the floor rests on the model reading a turn rightly, on core listing the words, and on the person who presses.

What this cannot do:

- A wish of somebody else's, in words core does not list. "My mum is after a park" is offered as a wish for a park, with the guess marked, in every look. The offer shows the clause, so that the person can see whose wish it is.
- A wish turned round, of a thing in words the rules have no phrase for, in words core does not list as doubt: "boozers on every corner would finish me off".
- A turn that stands beyond a mark from the thing, or in another sentence: "no, a park", "What I don't want: a park", "parks, no thanks", "A park? Never." A guess is held to what leads up to the thing as far back as a mark. "Add all" takes each of these but the last, which stands in a sentence that asks.
- A sign of doubt that is no word that turns, after the thing: "a park would be awful", "my ex wanted a park". The guess is marked, and "add all" leaves it.
...
- `tests/test_guard.py` holds the first three of these as tests that are expected to fail, so that the day one is closed the test says so. `tests/test_adversary.py` holds the words about wealth and identity as tests like any other, since core lists them.
...
|---|---|---|
| To avoid a group | "not too many students", "fewer immigrants" | No edit for that part. `policy_redirect`. The rest is applied, where the prompt is plain |
...
The `neutral_places` notice is one of two fixed texts, and `notice_text(notice, changed)` in core chooses. Where an edit of the same answer changed the spec: "Burro ranks places by what is there, such as schools, parks, venues and transport, and never by who lives there. The rest of your search has been applied." Otherwise the first sentence, and then: "Nothing you typed has changed your search." Each is the same for every group and every user. `changed` is the API's to say, from what the reducer applied.
...
```json
{"meta": {"release_id": "syn-2026-09-23-01", "engine_version": "1.12.0", "synthetic": true,
          "preview": false},
 "data": {}}
```
...
| 4 | `GET /v1/areas` | | `areas`: `area_id`, slug, name, borough, centroid, rankable. `bands`: for each vibe that holds `lens`, in shelf order, its `tag_id` and `marks`, one for each area by id, each an `area_id`, `band`, `spread_low` and `spread_high` | | Yes |
...
| `UnmetAt`, in `unmet_at` of route 1 | `category`, `span` | Where the words stand that ask for what nothing measures, where a model said which they are and they are in the text. Every category here is in `unmet` too |
...
| `model_pending` of route 1 | A boolean | True where `ask_model` was `false`, a model is on, and the rules left words unread. A client asks again, with `ask_model` left out or `true`, to have what the model adds. False in every other answer |
...
**What the document says of a field that has a default.** `TagWeight.toward` and `Assumption.word` have a default in core, so `contracts/openapi.json` does not list them as required. `toward` may be left out of a body, and is then `high`. Both are always returned. A `TagEdit` has no default: a body that leaves `toward` out is refused with 422 `invalid_operations`, `missing` at `operations.tag_ops[n].toward`. The id of a vibe that was retired is no `TagId`, so it is refused where it stands: `not_allowed` at `spec.tags[n].tag_id`, with `invalid_spec`. A vibe that is a `TagId` and that the release does not carry, the other way gritty was built, is `not_in_release`: from `check_spec` in a spec, and as a rejected edit in `operations`.
...
| Why a provider that was set is not used (`reason`), as one of seven fixed words | A provider's key, or anything else that was set: a value in the wrong variable could be a key |
...
**What is said of a provider is a fixed word, and never what was set.** Two lines name a provider, both at the level of a warning. `model_not_used` is written once, as the service starts, where something was set and no model reads: it holds `provider`, where what was set names one of the four, and `reason`, the first thing that does not hold: `no_provider`, `unknown_provider`, `no_key`, `unfit_key`, `terms_not_accepted`, `unfit_model` or `not_for_people`. With nothing set it is not written. `model_resting` is written each time an adapter begins to leave a provider alone, after three refusals in a row: it holds `provider` and nothing of any call. A provider that will not read what was sent gets no line of its own: the line of the call says `refused` in `call_status` (section 10.2). Each word is the value of an enum, so no line can hold a key, a model's name as it was set, or a word of a person's.
...
| core | `test_no_feature_or_tag_describes_residents`, `test_the_guard_lets_the_one_name_of_transport_noise_through_and_refuses_every_other` | A denylist of resident words against every id, label and definition. One name is let through, as the whole of a name: the share of residents that transport noise reaches |
...
| api | `tests/test_offers.py` | 8.2: what "add all" may add at one press, on every answer on disk |
...
| Where the hashes of an approved release are kept | Open, 2026-09-24. `hashes.json` stands beside the release, in the folder the build wrote. Until a release is approved nothing else holds its hashes, so a person who changes a release can change its hashes with it, and `open_served` cannot tell. To approve a release is to commit its lock and its hashes (ADR 0018). Where they are committed, and how the service is told which hashes were approved, is not decided. Until it is, a preview is held by `burro-release check`, which holds every figure to its row of evidence, and by whoever builds it |
...
| A firm budget held against a median | **The founder's to decide** (ADR 0021). Half of the homes behind a median sold for no more than it, so an area that a firm budget leaves out may hold homes that sold within it. A firm budget could rank such an area lower and never leave it out. As built, it leaves it out (section 6.5) |
| What a median rests on | Open, 2026-09-24. `unstated`, because the publisher gives no count of sales with it. It gives the count as a dataset of its own, which is registered and not yet read. With it a row could say `high` or `medium`. No word of `confidence` is for a figure of 5 to 9 sales |
| A price for a home of any kind | Open, 2026-09-24. The publisher gives one, and a budget names a kind of home, so no row holds it. It has a place only if `Segment` gains one for it |
...
| `NEVER_A_TRADE_OFF` | Ten minutes on foot, 800 m and 1,600 m, chosen by judgement (section 7.5). Every distance is a straight line since catalogue version 12, so the walk is longer than the figure, and each floor is to be looked at again. No published standard was read for them. With them a result shows a trade-off less often: a short walk that most areas better is in the working and in no sentence |
...
| The sources of the 21 new features | Section 3.1 names the source of each that a build works out. Four are in no build: `private_outdoor_space` and `cuisine_variety` have no source that is cleared, and `gp_walk` and `pharmacy_walk` have a step that stops at its first real file |
...
| A release whose content changes under its id | The `ETag` is the release id. The synthetic release was rebuilt under `syn-2026-09-23-01` while version 2 was built, and again on 2026-09-24 when the plan of the city was redrawn (section 2.9), so a browser that held the older answer would be told it still stands. Nothing is deployed, so nothing is affected. A release never changes once it is published (section 2), and a rebuilt fixture is the one case where it does: a browser used for development must be reloaded with its cache off after `make fixture` |
