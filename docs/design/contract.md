...
| `catalogue.json` | `{"catalogue_version": 13, "metrics": [...], "vibes": [...]}` | a feature this release carries, and a vibe it carries | 2.3 |
...
| `sources` | list | `source_id`, `name`, `publisher`, `licence`, `attribution`, `url`, `retrieved_on`, `credit_beside_figures`. Every `source_id` used anywhere in the release appears here once. `credit_beside_figures` is true where the publisher asks that its `attribution` stands wherever a figure made from its data is shown, and not on the page of attributions alone: the licence registry says which, and every fact that cites the source then carries the statement (section 7.1). It is false where a release does not say |
...
| `since` | month or null | The first month the sales were made in. It is given exactly where `sales` is |

**A cost is a range, or one number where no range is known.** A row is one of three things, and says which by what it holds.
...
| `private_outdoor_space` | homes | Addresses with private outdoor space | More addresses with outdoor space | % | more | amenity | buildings | streets_homes | no | more / fewer | `ons-access-to-garden-space-2020`, held to `ons-msoa11-msoa21-lad22-lookup`. Held back from every release until its row of the proxy audit has passed |
...
- **Every one of them is of the place.** Each counts shops. None counts a person, and the name of none holds a word for who lives somewhere: a chain of gyms whose name holds such a word is said by the rest of its name. What the mix follows is measured, and written down, before it is served: the row of the proxy audit is in `docs/research/data/brands.md`.
...
`raw = sum(w_i * u_i) / sum(w_i)` over the parts whose feature has a value, and `coverage = sum(w_i present)`. If `coverage < 0.6` then `raw`, `score` and `band` are null. So are they for a vibe of `PLACED_ONLY_WITH` where none of the parts named for it has a value, whatever `coverage` is: Village feel, below. `score` is the percentile of `raw` and `band` its band, by section 2.4. `tag_raw()` in core is the only implementation; the pipeline calls it. It rounds `raw` to six decimals, the precision a release is written with, so the number is the same before writing and after reading.
...
**Village feel places an area only where something of its town centre has a figure.** Decided on 2026-09-24. With independent places measured, a build of London holds 60 in 100 of its recipe: independent places, homes built before 1919 and conservation cover. That is enough for a band, and none of it is of a town centre. On it Village feel found inner London's old streets and no villages. So it places an area only where `centre_small` or `centre_compact` has a figure, whatever else has. `PLACED_ONLY_WITH` in `catalogue.py` names the two, and `tag_raw()` holds the rule, so a release that places an area without one is refused (`raw_matches_recipe`). Its `coverage` is said as ever, and the sentence of the vibe says how many of its parts are known. No build of London carries either measure, so no build places an area on Village feel, and a wish for it there is turned away as `not_in_release`. The made-up release carries both, and no figure of it moved. It is served once a second try reads as villages, and nothing is taken out of `PLACED_ONLY_WITH` but in a change the founder has seen.
...
WITHIN_BY                         = 5      likely within: at least this many minutes under the limit
...
| `sources` | tuple of `FactSource` | `source_id`, `name`, `publisher` and `attribution`, from the manifest, in `source_id` order. Never empty. `attribution` is the publisher's own statement of credit, where the release says it stands beside every figure (`credit_beside_figures`, section 2.1), and `null` for any other source. A client draws it with the source wherever the fact is shown |
...
| A journey, as a guide, to a place named in full | Recorded crime, in any form, anything that counts who lives somewhere, and anything the rules offer with a note |
...
```json
{"meta": {"release_id": "syn-2026-09-23-01", "engine_version": "1.13.0", "synthetic": true,
          "preview": false},
 "data": {}}
```
...
| The sources of the 21 new features | Section 3.1 names the source of each that a build works out. One is in no build: `cuisine_variety` has no source that is cleared. `private_outdoor_space` has a source that is cleared and is worked out where a build names its files, and it is held back from every release until its row of the proxy audit has passed. The made-up release does not carry `gp_walk` or `pharmacy_walk`, which a build of London does |
