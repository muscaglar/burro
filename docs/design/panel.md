...
| `GET moved` | | What differs between the release that is shown and one other, which is named as the desk starts: what came and went of the catalogue itself, the areas, measures and vibes that came or went, how far the figures of each measure moved, how many areas changed band on each vibe, which files behind the two builds differ, and the first ten areas of the three searches, before and after. With no other release named, one line that says how to name one |
...
**The other release may be a build of another catalogue.** That is when the screen is most needed: after a measure or a vibe was added, or a recipe changed. The release that is shown is read as it is served, as every screen of the panel reads it. The other is read as the step reads it, by its own catalogue, and is held against and no more: no screen shows a figure of it as served. The screen says the version of the catalogue of each, and first of all what came and went of the catalogue itself: a name or a label that changed, and of each vibe both carry, the parts of its recipe that came or went, the shares that changed, and whether it became a rough guide or ceased to be one. A search is ranked on each release by what that release holds, and what it was ranked without is said of the release it is so of.
