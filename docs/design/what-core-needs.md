...
| `station_walk` | Label "Straight-line distance to the nearest way in to a station". Unit m. Resolution point. Never a trade-off under 800 m. An offer of it says that within 800 m in a straight line is about a 10 to 15 minute walk | It is the heaviest weight a search starts with, in `spec.py`. The founder says whether the cable car is a station |
...
Every floor of 800 m was chosen for a walk of ten minutes, and a straight line is shorter than the walk. Each is to be looked at again. The floor of a station was looked at on 2026-09-25, and stands: the founder decided that near a station is about a 10 to 15 minute walk, which a straight line of 800 m is.
...
A third is of high streets, and core holds a feature for it since catalogue version 14. It was built for a second try at Village feel, which did not clear the founder's bar: [the page on high streets](../research/data/high-streets.md) says what it found. The founder chose on 2026-09-25 to serve Village feel on it all the same, as a rough guide (ADR 0013, as amended).
...
|---|---|---|
| `private_outdoor_space` | **On the table, named, and carried.** Core names it as it is built since version 13: "Addresses with private outdoor space", by MSOA. Its workbook is keyed by the census areas of 2011, and every figure is held to the statistics office's lookup between the areas of 2011 and of 2021, which has its receipt: 963 of London's 1,002 areas are marked as unchanged and have a figure, 38 are parts of a split and one was made by joining two. It was held back from every release for want of a row of the proxy audit, and for no other reason. The founder dropped the audit on 2026-09-25 ([decision record 0006](../adr/0006-rank-places-not-residents.md), as amended), so `HELD_BACK` in its module is empty, and it is 25 in 100 of Houses or flats | Nothing. Whether a split is carried is the founder's: it is one line, `CARRIED` in `derive/areas_of_2011.py`, and would give 38 more areas a figure |

## 2. Measures of who lives somewhere

**Made.** Four are worked out from two census tables: `residents_aged_20_34`, `residents_aged_65_over`, `households_dependent_children` and `households_one_person`. Core holds each, under the name it is built with, and each is on the table of measures, so a build that names the list of the two tables carries them. Two vibes hold one each: section 4. The row of the proxy audit was written first: [the design](residents-age-and-households.md), section 12. No audit has run, and none is to: the audit was dropped on 2026-09-25, and the row holds nothing back.
...
| Village feel | **Made, as a rough guide.** The founder chose on 2026-09-25 to serve it though no try had reached the bar, which is that 30 of the fifty areas a recipe puts highest read as villages: three tries put 8, 24 and 25 there. It is served on the recipe of the second try, whose largest part is how much of the nearest high street lies in a conservation area, and core names that measure. It says wherever it is shown that it is a rough guide, and why, and it is never taken without a press of its own. `PLACED_ONLY_WITH` names no vibe. What would make it as sure as the rest is the founder's reading of the fifty, and a figure that tells a village street from a trunk road | Section 1, [the contract](contract.md), section 3.2, [the page on brands](../research/data/brands.md), section 7, and [the page on high streets](../research/data/high-streets.md) |
...
- Do not hold a measure back for want of a row of the proxy audit. The audit was dropped on 2026-09-25 (decision record 0006, as amended), and the four rows that were written hold nothing back. What keeps residents out of a ranking is unchanged: section 2.

## 6. What the join left to do

The join is made. The words for how well off a place is are core's, with two readings, both of the place. The guard on a model stands over the reader of a whole search. The test of the areas most like an area holds both mends. Every generated file was made again. The decision record that a branch of that join had numbered 0030, on what is typed going to a provider as typed, was given the number 0023 when it was joined, and is [0023](../adr/0023-what-is-typed-goes-as-typed-and-people-are-told.md). It is not the record that bears the number 0030 since 2026-09-25, which is of how a release is kept, approved and carried.
