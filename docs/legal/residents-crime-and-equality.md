...
| A person who uses Burro, wronged by a rule that falls harder on people like them | A setting that nobody chose, or a measure, that ranks lower the areas where people of one group mostly live | Not known. It is what the proxy audit was for. The audit was never built, and the founder dropped it on 25 September 2026 ([ADR 0006](../adr/0006-rank-places-not-residents.md)), so nothing looks for it. `[SOLICITOR]` |
...
| 8 | Whether a measure with no word about residents still falls harder on one group | Not known | Every measure that is built | A measure would be dropped, capped or made to count only on request. The proxy audit was to say which, and was dropped on 25 September 2026 |
