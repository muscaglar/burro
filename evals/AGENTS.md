...
- **Every sentence is made up, but one.** A real user's words never enter this folder, reworded or not, without their consent in writing (ADR 0005). This repository is public. `whole-035` is the founder's own test sentence, word for word, and the founder consented to it.
...
- Places and areas are names of the synthetic release. `score.py --check` refuses any other in what a case expects. The words of `whole-035` name a place the release does not hold, which the reader must ask about.
- Where two readings of a sentence are fair, the case accepts both. It does not pick one.
- `score.py` and `controls.py` import the standard library and `burro_core` only. `stand_in.py` and `replay.py` are the two files that import the API, and neither calls a provider. `score.py --reader model` asks the API for its reader by name, and loads nothing of it until then.
