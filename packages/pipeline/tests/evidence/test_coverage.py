...
def test_every_area_and_every_measure_has_a_state():
    coverage = covered()
    assert len(coverage.areas) == 24 and len(coverage.measures) == 144
    assert len(coverage.cells) == 24 * 144
...
def test_a_release_is_held_to_every_measure_a_result_needs():
    ...
    assert kinds == {"area": 2, "travel": 3, "cost": 10, "station": 1, "feature": 114, "tag": 14}


def test_the_gaps_the_synthetic_release_has_on_purpose_are_counted():
    coverage = covered()
    assert len(coverage.gaps) == 339
    assert Counter(cell.state for cell in coverage.gaps) == {
        # 177 features and 21 vibes with too little covered. Twelve of the features are the
        # four shares of who lives there, in the three areas where too few were counted.
        State.BELOW_THRESHOLD: 198,
...
def test_the_reason_a_publisher_gives_is_kept():
    ...
    assert len(suppressed.gaps) == 339
    ...
    assert len(unpublished.gaps) == 338
...
def test_the_report_has_the_five_tables_and_a_row_for_every_gap():
    ...
    # Of 339 gaps, 96 are of the four measures no area has. Each of those is said once.
    assert len([line for line in gaps.splitlines() if line.startswith("| syn-")]) == 339 - 96
...
def test_the_report_says_once_what_no_area_has_and_not_once_for_every_area():
    ...
    assert "Of the 144 things Burro measures, 130 have a figure in at least one area." in gaps
    ...
    assert len([line for line in gaps.splitlines() if line.startswith("| syn-")]) == 243 - 33
    whole = report(covered())
    assert "Of the 144 things Burro measures, 140 have a figure in at least one area." in whole
...
def test_what_a_build_prints_holds_counts_and_a_hash_and_names_no_area():
    ...
    assert line == (
        f"release={RELEASE_ID} areas=24 measures=144 values=3117 gaps=339 no_record=0 "
