...
def test_every_fact_is_named_when_there_is_no_row_at_all():
    ...
    assert len(found) == 3148 + 72
    assert counted(found) == {"fact_has_a_row": 3220}
...
def test_a_row_for_a_gap_is_not_a_fault():
    ...
    # 175 figures of a measure, 33 costs and 32 vibes that an area lacks, and the four
    # measures that no release carries yet, in each of 24 areas.
    assert len(gaps) == 175 + 33 + 32 + 4 * 24
...
def test_a_fact_that_cites_a_source_with_no_file_behind_it_is_named():
    ...
    assert len(found) == 3220
    assert counted(found) == {"source_has_a_file": 3220}
...
def test_a_row_that_rests_on_a_file_kept_for_the_audit_or_the_census_table_is_named(
    ...
    # Every measure, every vibe, every cost and every journey rests on the homes: 2,616,
    # 336, 240 and 72. A name does not, and nor does a measure that the release does not carry.
    assert len(rows) == 3264 and "lon-n0001/area/name" not in named
...
def test_a_fact_whose_evidence_is_kept_for_the_audit_or_the_census_table_is_named(
    ...
    # A fact is served only where there is a figure: 2,441 of a measure, 207 costs and 72
    # journeys. A vibe rests on the homes where it is placed, and where a part of it has a
    # figure: 304 and 21. And 115 facts of likeness were counted from the figures of measures.
    assert len(facts) == 2441 + 207 + 72 + 304 + 21 + 115
...
def test_a_made_up_file_for_the_audit_stands_behind_no_made_up_figure_either():
    ...
    assert found == {"evidence_is_for_the_product": 3160, "input_is_for_the_product": 3264}
...
def test_a_row_that_rests_on_a_file_the_registry_does_not_allow_for_its_use_is_named(
    ...
    # 109 measures and 14 vibes, in each of 24 areas.
    assert {row.split("/")[1] for row in rows} == {"feature", "tag"} and len(rows) == 123 * 24
    assert str(found[0]) == (
...
def test_a_row_that_rests_on_a_file_fetched_for_an_internal_use_is_named(use: str):
    ...
    assert counted(found) == {"input_is_allowed": 2952}
    ...
    assert counted(unevidenced(release(), made_up)) == {"input_is_allowed": 2952}
...
def test_a_release_whose_source_is_no_longer_allowed_for_scoring_is_found():
    ...
    # The rows of 2,952 measures and vibes, and of 240 costs. A journey is put to another use.
    assert len(findings) == 3192
