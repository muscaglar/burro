...
def test_every_fact_is_named_when_there_is_no_row_at_all():
    ...
    assert len(found) == 3169 + 72
    assert counted(found) == {"fact_has_a_row": 3241}
...
def test_a_row_for_a_gap_is_not_a_fault():
    ...
    # 178 figures of a measure, 33 costs and 32 vibes that an area lacks, and the four
    # measures that no release carries yet, in each of 24 areas.
    assert len(gaps) == 178 + 33 + 32 + 4 * 24
...
def test_a_fact_that_cites_a_source_with_no_file_behind_it_is_named():
    ...
    assert len(found) == 3241
    assert counted(found) == {"source_has_a_file": 3241}
...
def test_a_row_that_rests_on_a_file_kept_for_the_audit_or_the_census_table_is_named(
    ...
    # Every measure, every vibe, every cost and every journey rests on the homes: 2,640,
    # 336, 240 and 72. A name does not, and nor does a measure that the release does not carry.
    assert len(rows) == 3288 and "lon-n0001/area/name" not in named
...
def test_a_fact_whose_evidence_is_kept_for_the_audit_or_the_census_table_is_named(
    ...
    # A fact is served only where there is a figure: 2,462 of a measure, 207 costs and 72
    ...
    assert len(facts) == 2462 + 207 + 72 + 304 + 21 + 115
...
def test_a_made_up_file_for_the_audit_stands_behind_no_made_up_figure_either():
    ...
    assert found == {"evidence_is_for_the_product": 3181, "input_is_for_the_product": 3288}
...
def test_a_row_that_rests_on_a_file_the_registry_does_not_allow_for_its_use_is_named(
    ...
    # 110 measures and 14 vibes, in each of 24 areas.
    assert {row.split("/")[1] for row in rows} == {"feature", "tag"} and len(rows) == 124 * 24
    assert str(found[0]) == (
...
def test_a_row_that_rests_on_a_file_fetched_for_an_internal_use_is_named(use: str):
    ...
    assert counted(found) == {"input_is_allowed": 2976}
    ...
    assert counted(unevidenced(release(), made_up)) == {"input_is_allowed": 2976}
...
def test_a_release_whose_source_is_no_longer_allowed_for_scoring_is_found():
    ...
    # The rows of 2,976 measures and vibes, and of 240 costs. A journey is put to another use.
    assert len(findings) == 3216
