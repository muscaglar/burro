...
def test_a_figure_with_no_row_of_evidence_fails_the_check_and_is_written_down(walk: Walk):
    ...
    assert f"step=check status=failed release={RELEASE_ID} facts=3241 rows=3456 " in said.lines[0]
...
def test_a_figure_that_rests_on_a_file_the_registry_does_not_allow_for_it_fails_the_check(
    ...
    assert f"step=check status=failed release={RELEASE_ID} facts=3241 " in said.lines[0]
    # The rows of 110 measures and 14 vibes, in each of 24 areas.
    assert " findings=2976 " in said.lines[0]
    assert said.lines[0].endswith(" input_is_allowed=2976")
    found = walk.findings.read_text(encoding="utf-8").splitlines()
    assert len(found) == 2976
    assert (
