...
def test_an_operation_is_named_for_its_route(document: dict[str, Any]):
    ...
    groups = (interpret, rank, areas, places, shares, meta)
...
def test_every_response_is_documented_with_the_synthetic_flag(document: dict[str, Any]):
    for _, path, operation in operations(document):
        if path == "/healthz":
            continue
    ...
    assert set(meta["required"]) == {"release_id", "engine_version", "synthetic"}
